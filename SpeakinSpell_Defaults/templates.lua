...
SpeakinSpell:PrintLoading("defaults/templates.lua")
...
function SpeakinSpell:Template_BuildAuto_Random()
	local funcname = "Template_BuildAuto_Random"
	...
	for _,name in pairs(Factions) do
		if (name ~= nil) and (name ~= "") then
			self:DebugMsg(funcname, "randomfaction:"..tostring(name))
	...
	for _,name in pairs(Factions) do
		if (name ~= nil) and (name ~= "") then
			...
			local enemy = self:FormatSubs(L["Enemy of <name>"], subs)
			self:DebugMsg(funcname, "randomtaunt:"..tostring(enemy))
			tinsert( t.Content.RandomSubs["randomtaunt"], enemy )
		end
	end
