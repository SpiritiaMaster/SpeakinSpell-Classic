		...
		SelectGroup = {
			...
			args = {
				...
				CreateSpellButton = {
					...
					func = function() SpeakinSpell:ShowCreateNew() end,
				},
			},
		},
--		Gap2 = {
...
--						}, --L["Edit the random speeches used for this Speech Event announcement"]
						...
						RPLanguage = {
							...
							--		use nil instead for Common in SendChatMessage
							values = function()
								local values = {}
								-- [internal value] = "display value"
								-- make sure they go in this order
								values[ "COMMON" ] = GetDefaultLanguage("player") -- could say "Orcish"
...
This option is not recursive, so if you want nested macros to expand sub-macro calls, you must enable this option in the separate settings for the nested macro events.
		...
		-- WoW 4.2.0 KLUGE: the scroll bar is not going down far enough for some people sometimes
		-- so to work around the problem, I'm putting several blank lines at the bottom
		ScrollGap = {
			order = 1000,
			type = "description",
			name = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
...
Duplicates of speeches listed above will not be accepted]]
				...
				GotoMacroGap = {
					...
					hidden = function()
						...
						local ete = SpeakinSpell:GetETEForSSMacro(msg, nil)
						if not ete then
							return true
						end
						return false
				...
				ExpandMacro = {
					...
					func = function()
						...
						ete.Messages = SpeakinSpell:ExpandOneSSMacro( ete.Messages, i, ete.DetectedEvent )
					end,
				},
			},
		}
...
function SpeakinSpell:CurrentMessagesGUI_OnMessage(getset, index, value, UseMultiLine)
	...
	if "GET" == getset then
		...
		EventTableEntry.Messages[ index ] = string.gsub( value, L["<newline>"], "\n" )
